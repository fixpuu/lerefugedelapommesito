...
export function WhyChooseMe() {
  ...
  return (
    <section
      ref={sectionRef}
      id="about"
      className="relative w-full py-24 md:py-32 bg-[#E8E0D0]"
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div ref={headerRef} className="text-center mb-16 md:mb-20 opacity-0">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-sans font-bold text-[#3D3229] tracking-tight">
            {whyChooseMeConfig.titleRegular} <span className="font-serif italic font-normal text-[#6B8E23]">{whyChooseMeConfig.titleItalic}</span>
          </h2>
        </div>
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-6 md:gap-8">
          ...
          {whyChooseMeConfig.featureCards.map((card, index) => (
            <div key={index} className="feature-card-image opacity-0 group">
              <div className="relative aspect-[3/4] rounded-xl overflow-hidden bg-[#3D3229]">
                <img
                  src={card.image}
                  alt={card.imageAlt}
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#3D3229]/80 via-transparent to-transparent" />
                <div className="absolute bottom-6 left-6 right-6">
                  <p className="text-[#FAF8F3] font-sans font-semibold text-lg mb-2">
                    {card.title}
                  </p>
                  <p className="text-[#FAF8F3]/70 font-body text-sm">
                    {card.description}
                  </p>
                </div>
              </div>
          ...
          {whyChooseMeConfig.stats.length > 0 && (
            <div
              ref={statsRef}
              className="feature-card-stats opacity-0 bg-[#FAF8F3] rounded-xl p-8 md:p-10 flex flex-col justify-between"
            >
              <div>
                {whyChooseMeConfig.statsLabel && (
                  <p className="text-[#6B8E23] text-sm font-body uppercase tracking-widest mb-8">
                    {whyChooseMeConfig.statsLabel}
                  </p>
                )}
                <div className="space-y-8">
                  {whyChooseMeConfig.stats.map((stat, index) => (
                    <div key={index} className="border-b border-[#3D3229]/10 pb-6 last:border-0">
                      <p className="text-4xl md:text-5xl font-sans font-bold text-[#3D3229] tracking-tight">
                        <Counter
                          end={stat.value}
                          suffix={stat.suffix}
                      ...
                      <p className="text-[#8B7355] font-body text-sm mt-1">
                        {stat.label}
                      </p>
                    </div>
                  ))}
        ...
        {whyChooseMeConfig.wideImage && (
          <div ref={wideRef} className="mt-16 md:mt-24 relative rounded-xl overflow-hidden group opacity-0">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-[#3D3229]/60 via-transparent to-transparent" />
            {(whyChooseMeConfig.wideTitle || whyChooseMeConfig.wideDescription) && (
              <div className="wide-text-overlay absolute bottom-8 left-8 md:bottom-12 md:left-12 max-w-md opacity-0">
                {whyChooseMeConfig.wideTitle && (
                  <p className="text-[#FAF8F3] font-sans font-bold text-2xl md:text-3xl mb-3">
                    {whyChooseMeConfig.wideTitle}
                  </p>
                )}
                {whyChooseMeConfig.wideDescription && (
                  <p className="text-[#FAF8F3]/70 font-body text-sm md:text-base">
                    {whyChooseMeConfig.wideDescription}
                  </p>
                )}
              </div>
