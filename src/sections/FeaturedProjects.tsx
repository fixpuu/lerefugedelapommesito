        ...
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end md:justify-between mb-16 md:mb-20 opacity-0">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-sans font-bold text-[#3D3229] tracking-tight">
              {featuredProjectsConfig.titleRegular} <span className="font-serif italic font-normal text-[#6B8E23]">{featuredProjectsConfig.titleItalic}</span>
            </h2>
          </div>
          {featuredProjectsConfig.viewAllText && (
            <a
              href={featuredProjectsConfig.viewAllHref}
              className="mt-6 md:mt-0 inline-flex items-center gap-2 text-[#5C4D3C] hover:text-[#6B8E23] font-body text-sm transition-colors duration-300 group"
            >
              {featuredProjectsConfig.viewAllText}
        ...
        <div ref={projectsRef} className="space-y-20 md:space-y-32">
          {featuredProjectsConfig.projects.map((project, index) => (
            ...
            >
              ...
              <div
                className={`project-image-wrap relative overflow-hidden rounded-xl group cursor-pointer opacity-0 ${
              ...
              >
                ...
                {/* Hover gradient */}
                <div className="absolute inset-0 bg-gradient-to-t from-[#3D3229]/50 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              </div>
              ...
              <div className={`project-content opacity-0 ${index % 2 === 1 ? 'md:order-1 md:text-right' : ''}`}>
                <div className={`project-text-item flex items-center gap-3 mb-4 ${index % 2 === 1 ? 'md:justify-end' : ''}`}>
                  <span className="text-[#8B7355] font-body text-sm">{project.category}</span>
                  <span className="w-1 h-1 rounded-full bg-[#D4A574]" />
                  <span className="text-[#8B7355] font-body text-sm">{project.year}</span>
                </div>
                <h3 className="project-text-item text-2xl md:text-3xl lg:text-4xl font-sans font-bold text-[#3D3229] tracking-tight mb-4">
                  {project.title}
                </h3>
                <p className="project-text-item text-[#5C4D3C] font-body text-base md:text-lg leading-relaxed mb-6">
                  {project.description}
                </p>
                {featuredProjectsConfig.viewProjectText && (
                  <a
                    href="#contatti"
                    className={`project-text-item inline-flex items-center gap-2 text-[#6B8E23] font-body text-sm border-b border-[#6B8E23]/30 pb-1 hover:border-[#6B8E23] transition-colors duration-300 group/link ${
                      index % 2 === 1 ? 'md:flex-row-reverse' : ''
                    }`}
                  >
                    {featuredProjectsConfig.viewProjectText}
